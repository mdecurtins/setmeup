...
**Mechanics:** a dedicated `.github/workflows/agent-review.yml` triggers on `pull_request_target` (runs TRUSTED code from the default branch, checks out only the base SHA) and opts into the `setmeup_ci` environment housing the only secret, `OPENROUTER_API_KEY`. `.github/scripts/agent-review.py` reads the PR diff + OpenSpec change artifacts via the GitHub API (never from the working tree — the PR head is never checked out or executed) and sends them (with the review checklist from `docs/workflow.md`/`PR_TEMPLATE.md`) to an OpenRouter model. Because the workflow has no `needs`, the script polls (bounded, ~20 min) for the four quality checks to settle on the PR head SHA, and only considers approving when they are all SUCCESS. The check's exit code IS the gate: approve → exit 0 (green, mergeable); reject or any fail-closed condition → exit 1 (red, merge blocked) + a verdict comment.
**Security posture (public repo, hardened by the reviewer's own first-run finding):** `pull_request_target` + a hard fork guard means fork PRs are skipped entirely (external contributions use the human review gate — a deliberate, documented limitation), and a same-repo branch PR can NEVER modify the reviewer workflow/script or reach the OpenRouter key, because the executed code is the default branch's trusted copy. The system prompt declares PR-controlled text (title/body/diff/artifacts) to be UNTRUSTED evidence and instructs the model to ignore instructions inside it (prompt-injection countermeasure). `GITHUB_TOKEN` does only read-only evidence + the reject comment — it never posts approvals (there are none). The script fails closed: no settled green quality checks, missing key, malformed LLM verdict, or any exception → red check.
**Sequencing:** the workflow and script ship now, dormant behind `vars.AGENT_REVIEWER_ENABLED=='true'` (so CI stays quiet until provisioned); the maintainer sets the env var + `OPENROUTER_API_KEY` secret, and after this change merges, observes a live pass on a subsequent PR (pull_request_target only fires from `main`), then adds `agent-review` to the required status checks on `main` (one `gh api` PATCH, `tasks.md` 7.4). Until then the four quality checks remain the required gate.
...
6. Add `.github/workflows/agent-review.yml` (`pull_request_target`, base-SHA checkout) + the `.github/scripts/agent-review.py` script (dormant behind `AGENT_REVIEWER_ENABLED`)
