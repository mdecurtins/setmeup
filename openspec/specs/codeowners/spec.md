...
## Purpose
Define ownership of trust-boundary and governance paths via a CODEOWNERS file, signaling governance awareness even in a single-maintainer repo.
...
- **THEN** the PR still merges normally (the owner's review is a signal, not a mechanically required one — `require_code_owner_reviews` is off because a personal-repo code owner is the author; the mechanically enforced review gate is the `agent-review` check)
